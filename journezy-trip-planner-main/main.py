...
async def plan_trip(request: TripRequest):
    try:
        ...
        # Handle workflow output - workflow returns markdown text
        print("✅ [MAIN] Workflow completed successfully")
        document_data: Optional[str] = None
        document_type: str = "markdown"  # Workflow returns markdown

        try:
            if isinstance(result, str):
                # Workflow returns markdown text directly
                print("📝 [MAIN] Received markdown itinerary from workflow")
                document_data = result
                document_type = "markdown"
                print(f"✅ [MAIN] Markdown content ready ({len(document_data)} characters)")
            else:
